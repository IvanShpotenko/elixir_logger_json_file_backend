  ...
  defp log_event(level, msg, ts, md, %{path: path, io_device: io_device, inode: inode, metadata: keys, json_encoder: json_encoder, triming: triming, uuid: uuid}=state) when is_binary(path) do
    if !is_nil(inode) and inode == inode(path) do
      message = case uuid do
        true ->
          json_encoder.encode!(Map.merge(%{level: level, uuid: UUID.uuid4(), message: (msg |> IO.iodata_to_binary), timestamp: format_time(ts)}, take_metadata(md, keys, triming))) <> "\n"
        false ->
          json_encoder.encode!(Map.merge(%{level: level, message: (msg |> IO.iodata_to_binary), timestamp: format_time(ts)}, take_metadata(md, keys, triming))) <> "\n"
      end
